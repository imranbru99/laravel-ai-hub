...
class StudioController extends Controller
{
    ...
    protected function playgroundPending(array $validated): \ImranDevBd\AiHub\PendingRequest
    {
        ...
        if (array_key_exists('thinking', $validated) && $validated['thinking'] !== null && $validated['thinking'] !== '') {
            $budget = is_numeric($validated['thinking']) ? (int) $validated['thinking'] : (bool) $validated['thinking'];
            $pending->thinking($budget);
        }
        if (! empty($validated['reasoning_effort'])) {
            $pending->reasoningEffort($validated['reasoning_effort']);
        }
