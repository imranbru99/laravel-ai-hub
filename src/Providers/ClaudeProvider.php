...
class ClaudeProvider extends AbstractProvider
{
    ...
    public function complete(array $payload): AiResponse
    {
        ...
        $thinking = $this->formatThinking($payload['thinking'] ?? null, $maxTokens, $temperature);
    ...
    protected function formatThinking(mixed $thinking, int &$maxTokens, ?float &$temperature): ?array
    {
        if ($thinking === null || $thinking === false || $thinking === 0) {
            return null;
        }

        $budget = is_int($thinking) ? $thinking : (is_numeric($thinking) ? (int) $thinking : 2048);
        if ($thinking === true) {
            $budget = 2048;
        } elseif (is_array($thinking)) {
            return $thinking;
        }
