    ...
    public function complete(array $payload): AiResponse
    {
        ...
        if (array_key_exists('thinking', $payload)) {
            ...
            if ($budget === false || $budget === 0) {
                ...
                $generation['thinkingConfig'] = ['thinkingBudget' => 2048];
            } elseif (is_array($budget)) {
                $generation['thinkingConfig'] = $budget;
            }
        }
        ...
        $answerParts = $parts->reject(fn ($part) => ! empty($part['thought']));
        $content = $answerParts->pluck('text')->filter()->implode('');
    ...
    public function stream(array $payload): \Generator
    {
        ...
        while (! $bodyStream->eof()) {
            ...
            while (($pos = strpos($buffer, "\n")) !== false) {
                ...
                $firstPart = data_get($json, 'candidates.0.content.parts.0');
                if (is_array($firstPart) && empty($firstPart['thought'])) {
