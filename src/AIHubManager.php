...
class AIHubManager
{
    ...
    /**
     * Get available or popular models for a provider (or all providers).
     */
    public function models(?string $provider = null): array
    {
        $popular = (array) config('ai-hub.popular_models', []);

        if ($provider !== null) {
            return (array) ($popular[$provider] ?? []);
        }
    ...
    /**
     * Inspect capability flags for a given model (reasoning, thinking, sampling, etc.).
     */
