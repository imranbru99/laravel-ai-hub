...
<main class="relative mx-auto max-w-6xl px-4 py-6 sm:px-6 animate-fade-up">
    ...
    <section x-show="activeTab==='playground'" class="space-y-6">
        <div class="panel rounded-2xl p-5 sm:p-6">
            ...
            {{-- Hybrid Thinking / Reasoning controls --}}
            <div x-show="modelSupportsThinking() || modelSupportsReasoningEffort()" 
                 class="mb-4 rounded-xl border p-3 flex flex-wrap items-center justify-between gap-3 text-xs"
                 :class="theme==='dark' ? 'border-accent/30 bg-accent/5 text-slate-200' : 'border-accent/40 bg-accent/10 text-slate-800'">
                <div class="flex items-center gap-2">
                    <span class="flex h-2 w-2 rounded-full bg-accent animate-pulse"></span>
                    <span class="font-bold">Hybrid Reasoning & Thinking</span>
                    <span class="text-[11px] opacity-75" x-text="modelSupportsThinking() ? 'Gemini 3.8 / Claude 3.7 budget tokens' : 'OpenAI reasoning effort'"></span>
                </div>
                <div class="flex items-center gap-3">
                    <template x-if="modelSupportsThinking()">
                        <div class="flex items-center gap-2">
                            <label class="flex items-center gap-1.5 cursor-pointer font-medium">
                                <input type="checkbox" x-model="playground.thinking" @change="persistPlayground()" class="rounded border-accent text-accent focus:ring-accent">
                                <span>Enable Thinking</span>
                            </label>
                            <select x-show="playground.thinking" x-model.number="playground.thinkingBudget" @change="persistPlayground()"
                                    class="rounded-lg border px-2 py-1 text-xs"
                                    :class="theme==='dark' ? 'border-white/10 bg-ink-900 text-white' : 'border-slate-200 bg-white text-slate-900'">
                    ...
                    <template x-if="modelSupportsReasoningEffort()">
                        <div class="flex items-center gap-1.5">
                            <span class="font-medium">Effort:</span>
                            <select x-model="playground.reasoningEffort" @change="persistPlayground()"
                                    class="rounded-lg border px-2 py-1 text-xs"
                                    :class="theme==='dark' ? 'border-white/10 bg-ink-900 text-white' : 'border-slate-200 bg-white text-slate-900'">
        ...
        <div class="panel rounded-2xl p-5 sm:p-6 min-h-[180px]">
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" x-show="playground.reply && playground.reply !== '…'" @click="navigator.clipboard.writeText(playground.reply); notify('Copied reply to clipboard!')"
                            class="inline-flex items-center gap-1 rounded-lg border px-2 py-0.5 text-[11px] font-semibold transition"
...
function aiHubStudio(boot) {
    ...
    return {
        ...
        modelCaps() {
            ...
            const geminiThinking = /^gemini-(?:3\.[78]|2\.[05]-flash-thinking)/.test(id);
            const claudeThinking = /^claude-3-7-sonnet/.test(id) || id.includes(':thinking');
            const supportsThinking = geminiThinking || claudeThinking;
            return {
                temperature: !openaiReasoning && !deepseekReasoning,
                max_completion_tokens: openaiReasoning,
                reasoning: openaiReasoning || deepseekReasoning || supportsThinking,
        ...
        modelSupportsThinking() {
            return !!this.modelCaps().thinking;
        },

        modelSupportsReasoningEffort() {
            return !!this.modelCaps().reasoning_effort;
        },
        ...
        playgroundPayload() {
            ...
            if (this.modelSupportsThinking()) {
                payload.thinking = this.playground.thinking ? (this.playground.thinkingBudget || 2048) : 0;
            }
            if (this.modelSupportsReasoningEffort() && this.playground.reasoningEffort) {
                payload.reasoning_effort = this.playground.reasoningEffort;
            }
            return payload;
        },
